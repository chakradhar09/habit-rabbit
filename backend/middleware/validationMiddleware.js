...
const isWithinLast16Days = (dateStr) => {
...
const validateRecentDateParam = (paramName) => (req, res, next) => {
  ...
  if (typeof value !== 'string' || !isWithinLast16Days(value)) {
    return badRequest(res, 'Date must be within the last 18 days and use YYYY-MM-DD format.');
  }
...
const validateTaskCompletionDatePayload = (req, res, next) => {
  ...
  if (!isWithinLast16Days(normalizedDate)) {
    return badRequest(res, 'Completion date must be within the last 18 days.');
  }
